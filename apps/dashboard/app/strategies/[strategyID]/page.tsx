...
export default function StrategyDetailPage({
  ...
  const { summary, dailyPnl, bySymbol } = dashboardData!;
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <StatCard
          ...
          icon={DollarSign}
          trend={summary.totalRealizedPnl >= 0 ? "up" : "down"}
          className={
            summary.totalRealizedPnl >= 0 ? "text-emerald-500" : "text-rose-500"
          }
        />
        <StatCard
          title="Win Rate"
          value={formatPercent(summary.winRate * 100)}
          icon={Percent}
        />
        <StatCard
          title="Profit Factor"
        ...
        <StatCard
          title="Max Drawdown"
          value={formatPercent(summary.maxDrawdown ?? 0)}
          icon={TrendingDown}
              ...
              {dailyPnl.length === 0 ? (
                ...
                dailyPnl.map((day) => {
                  ...
                  const maxAbs = Math.max(
                    ...dailyPnl.map((d) => Math.abs(d.RealizedPnL))
                  );
