...
type Service struct {
	...
	// (sym, tf) anchor-timeframe callback registered by InitAggregators.
	...
	// Slots seeded by WarmUpNative; the aggregator path in WarmUp must skip
	// calc.Update for these or today's bars get fed twice.
...
func (s *Service) WarmUpNative(sym domain.Symbol, tf domain.Timeframe, bars []domain.MarketBar) int {
	...
	s.lastHTFSnaps[key] = lastSnap
	reg, _ := s.regimeDetector.Detect(lastSnap)
	s.anchorRegimes[key] = reg
	return len(bars)
}

// ReserveHTFNative pre-marks a (sym, tf) slot as canonically seeded by
// WarmUpNative. Must be called before Service.WarmUp runs — otherwise the
// 1m-warmup's HTF aggregator path drives calc.Update on today's pre-boot
// closes and WarmUpNative's later seed double-counts them.
...
func (s *Service) InitAggregators(symbols []domain.Symbol, sessionOpen time.Time) {
	...
	if s.shadowIndicator != nil {
		s.shadowIndicator.SetSessionOpen(sessionOpen)
		for key, unsub := range s.htfUnsubs {
			unsub()
			delete(s.htfUnsubs, key)
		}
		for _, sym := range symbols {
			for _, tf := range anchorTimeframes {
				key := sym.String() + ":" + tf.String()
				cb := s.makeHTFCallback(sym, tf)
				s.htfUnsubs[key] = s.shadowIndicator.Subscribe(sym, tf, cb)
			}
		}
	}
	for _, sym := range symbols {
