...
if docker compose --env-file "$ENV_FILE" -f "$MONITORING_COMPOSE" ps --status running 2>/dev/null | grep -q "omo-"; then
  info "Stopping monitoring stack..."
  docker compose --env-file "$ENV_FILE" -f "$MONITORING_COMPOSE" down
