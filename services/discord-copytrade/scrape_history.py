"""Discord channel history scraper.

One-shot: scrolls the target channel upward, deduping by message id, until
either the oldest visible message is older than --days or the scroller stops
making progress (hit channel start). Writes JSONL sorted oldest-first, one
message per line: {id, author, ts, text}.

Reuses the bootstrap session in state/storage_state.json. Run alongside the
watcher — both read-only against the state dir; Discord allows concurrent web
sessions on the same account.

Usage (inside docker):

    docker compose --profile scrape run --rm scrape \
        --days 90 --out /app/state/history.jsonl

DISCORD_CHANNEL_URL is read from .env.
"""
...
def _setup_logging(level: str) -> logging.Logger:
    ...
    return logging.getLogger("copytrade-scrape")


def snowflake_to_dt(message_row_id: str) -> datetime | None:
    # DOM id format: "chat-messages-<channel>-<snowflake>"
...
# JS that grabs every li[id^="chat-messages-"] currently rendered. The watcher
# uses a `.slice(-N)` variant; here we want the full buffer so the caller can
# accumulate across scroll ticks.
_EXTRACT_ALL_JS = r"""
(() => {
  const out = [];
...
# JS that scrolls the message list upward by a page and returns the scroller's
# new scrollTop. Returns -1 if no scroller could be located (DOM shape changed).
_SCROLL_UP_JS = r"""
(() => {
...
async def run(
    ...
    async with async_playwright() as pw:
        ...
        while True:
            ...
            # Identify oldest in-DOM message. Snowflake ordering == time order.
    ...
    # Normalize, filter to cutoff window, sort oldest-first, write JSONL.
    records: list[dict] = []
