...
### CLI binary that reimplements a library function will drift

Stronger version of "audit each entry-point's wire-up": **don't have multiple entry points to begin with**. When a `cmd/<name>/main.go` reimplements logic that already exists as a library function (e.g. `internal/app/<x>.NewRunner.Run()`), every new feature added to the library function silently becomes a bug in the CLI binary. The 2026-05-01 omo-replay session traced four patch PRs in 24 hours (#67 indicator subscriber wiring, #73 AVWAP shard-aware dispatch, then dark-pool / whale / HTF warmup) all rooted in the same SRP violation: omo-replay's `--backtest` mode was a ~400 LOC inline copy of `backtest.NewRunner.Run()`. Fix shape: cmd binary delegates with a thin wrapper that translates flags → RunConfig → `NewRunner(...).Run(ctx)`. Don't extract a "shared helper" that both call — that's still two callers and still drifts; one canonical implementation, every other entry point delegates. PR #74 collapsed `omo-replay --backtest` from inline to delegation; trade-list parity with HTTP `/backtest/run` jumped from 1/7 contracts matching to 7/7, P&L delta within 0.32%. Diagnostic signature: a single backtest config produces materially different trade counts on two paths that should be equivalent.

### Dynamic-watchlist event-driven strategies need a sentinel symbol, not `symbols = []`
A new strategy that intends to "lazy-init per ticker as signals arrive" is tempted to ship its TOML with `symbols = []` + `watchlist_mode = "dynamic"`. This silently breaks two ways: (1) live bootstrap iterates the empty list and registers zero instances — the bus subscription handler can't `findInstanceByStrategy` anything; (2) backtest's `symbolOverrideSpecStore.intersectSymbols` treats empty natives as "no preference" and substitutes the entire backtest universe, registering one instance per universe symbol — signals arm on a random instance whose `Symbols` list does not include the actual ticker, so the strategy state machine never sees the ticker's bars. Diagnostic signature: backtest log shows `signal armed instance_id=strategy:1.0.0:LINK/USD ticker=AAPL ...` (instance owns LINK/USD bars but armed AAPL), and trade count is zero despite signals firing. Fix shape: TOML uses a sentinel like `symbols = ["__yourstrategy__"]` (mirror copytrade); bootstrap registers exactly one instance per shard via the `isSentinelSymbol` exemption; the bus handler calls `Router.AddSymbol(instID, ticker)` on each signal so subsequent bars for that ticker route to the sentinel instance. Bar routing follows the watchlist union, not the universe slab. `symbolOverrideSpecStore` must also preserve sentinel symbols verbatim during intersection — a pure-sentinel native list returns unchanged; mixed lists keep sentinels and intersect the rest. Without the spec-store fix the sentinel gets stripped at backtest setup.

### Handler publishes a bus event but no Runner subscriber exists
A new HTTP/sidecar handler that publishes a strategy-specific bus event (e.g. `EventTradingTheTrendSignalReceived`) needs a matching `r.eventBus.Subscribe(ctx, T, r.handleT)` call inside `Runner.Start()` AND a translator method that converts the `domain.XPayload` to the strategy-package event type (`start.X{...}`) and dispatches via `Instance.OnEvent`. Forgetting either half leaves a silent gap: the event publishes, the bus accepts the publish, the strategy never sees it. Strategy unit tests pass because they call `Instance.OnEvent` directly and don't traverse the runner. Handler tests pass because they only assert `bus.Publish` was called. Diagnostic signature: integration replay/replay-with-fixtures shows zero strategy entries despite the sidecar emitting valid signals; tail of `omo-core` log shows `<X>SignalReceived accepted` without any matching `Instance.OnEvent` dispatch line. Fix shape: mirror the existing `handleCopytradeSignal` (subscribe in `Runner.Start`, translate payload, lazy-init per-key state if `watchlist_mode = "dynamic"`, dispatch). Test shape: a runner-level test in `internal/app/strategy/runner_<x>_test.go` that publishes the bus event and asserts a fake-strategy `onEventFn` saw the translated `start.X` — this is the only test layer that exercises the seam.
